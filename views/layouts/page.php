...
if (isset($pager) && is_object($pager)) {
    ...
    <ul class="pagination pagination-sm nomargin">
        ...
        <li<?php echo $pager->getCurrPage() <= 1 ? ' class=disabled' : ''; ?>>
            <a title="第一页" href="<?php echo $pager->getCurrPage() <= 1 ? 'javascript:void(0)' : url(request()->uri(), ['page' => 1]); ?>">&lt;&lt;</a></li>
        <?php if ($pager->hasPrePage()) { ?>
            <li><a href="<?php echo url(request()->uri(), ['page' => ($pager->getCurrPage() - 1)]); ?>" title="上一页">&lt;</a></li>
        <?php } else { ?>
        ...
        foreach ($near_pages as $near_page) { ?>
            ...
            <?php } else { ?>
                <li>
                    <a href="<?php echo url(request()->uri(), ['page' => $near_page]); ?>"><?php echo $near_page; ?></a>
                </li>
            <?php } ?>
        <?php } ?>
        <?php if ($pager->hasNextPage()) { ?>
            <li>
                <a title="下一页" href="<?php echo url(request()->uri(), ['page' => ($pager->getCurrPage() + 1)]); ?>">&gt;</a>
            </li>
        <?php } else { ?>
            <li class="disabled"><a href="javascript:void(0)">&gt;</a></li>
        <?php } ?>
        <li<?php echo $pager->getCurrPage() >= $pager->getTotalPage() ? ' class="disabled"' : '' ?>>
            <a title="最后一页" href="<?php echo $pager->getCurrPage() >= $pager->getTotalPage() ? 'javascript:void(0)' : url(request()->uri(),
                ['page' => $pager->getTotalPage()]); ?>">&gt;&gt;</a></li>
    </ul>
<?php } ?>